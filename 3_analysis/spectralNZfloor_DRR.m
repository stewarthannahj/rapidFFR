function spectralNZfloor_DRR(fileDirectory, listener, Active, Reference, Lcut_off, Hcut_off, technique, condition, draws, repeats, totalSweeps, s_trig_interval, prestim, F0, F1, F2, F3)
% with replacement
                    ...
                    for j = 1:2
                        ...
                        endTrig = EEG.event(1,2).latency + round((prestim/1000)*EEG.srate) + ((totalSweeps-2)*s_trig_interval); % don't look at final nRep
                            ...
                            elseif strcmp(condition,'phaselocked')
                                ...
                                % indices of random set of triggers
                                index = randperm(length(triggers));
                                index = index(1:TRIGdraws);
