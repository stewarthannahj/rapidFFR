...
lineData.add<-matrix(nrow=(16*length(chunks)*length(var_name)),ncol=11)
# name columns
...
for (h in 1:16){
  if(h==1){
    ...
    listener = "L16"
      ...
      dataName_add <- paste("out/Experiment1_normalVrapid/Routput_stdChunks/snr_",listener,"_stdChunks_",chunk,"_",var_name[i],"add_28July21.csv")
      dataName_sub <- paste("out/Experiment1_normalVrapid/Routput_stdChunks/snr_",listener,"_stdChunks_",chunk,"_",var_name[i],"sub_28July21.csv")
...
# save to data file      
write.csv(lineData.add,file=paste("out/Experiment1_normalVrapid/snr_stdChunks_concat_add.csv"))
write.csv(lineData.sub,file=paste("out/Experiment1_normalVrapid/snr_stdChunks_concat_sub.csv"))
