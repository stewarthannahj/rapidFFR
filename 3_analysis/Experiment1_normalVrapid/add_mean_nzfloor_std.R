# calculate mean for noise floor, add rows only (standard FFR)
...
random<-subset(add_tons,condition=="random") # random rows only for noise floor
...
zscores<-matrix(nrow=length(levels(random$listener))*length(levels(random$chunk))*2,
                ncol=10)
# name columns
...
write.csv(zscores,"out/Experiment1_normalVrapid/nzfloor_mean_std_add.csv",row.names=FALSE)
