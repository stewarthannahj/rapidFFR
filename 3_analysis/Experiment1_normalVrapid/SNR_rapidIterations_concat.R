...
for (h in 1:16){
  ...
  for(z in 1:length(ALLnReps)){
    nReps = ALLnReps[z]
      ...
      dataName_add <- paste("out/Experiment1_normalVrapid/Routput_rapidIterations/snr_",listener,"_rapid_",nReps,"_",var_name[i],"add_28July21.csv")
      dataName_sub <- paste("out/Experiment1_normalVrapid/Routput_rapidIterations/snr_",listener,"_rapid_",nReps,"_",var_name[i],"sub_28July21.csv")
      ...
      lineData.sub[row,4] <- "rapid"
      lineData.sub[row,5] <- var
...
# save to data file      
write.csv(lineData.add,file=paste("out/Experiment1_normalVrapid/snr_rapidIterations_concat_add.csv"))
write.csv(lineData.sub,file=paste("out/Experiment1_normalVrapid/snr_rapidIterations_concat_sub.csv"))
